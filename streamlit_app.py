...
try:
  ...
  if not fruit_choice:
    ...
    streamlit.dataframe(back_from_function)                                                      # display as nice table
...
if streamlit.button('Get Fruit List'):
  ...
  my_data_rows = my_data_rows.sort_values()
  my_cnx.close()
...
def insert_row_snowflake(new_fruit):
  with my_cnx.cursor() as my_cur:
    my_cur.execute("insert into pc_rivery_db.public.fruit_load_list values ('" + new_fruit + "')") # do insert
    return "Thanks for adding " + new_fruit                                                        # show what user put in

add_my_fruit = streamlit.text_input('What fruit would you like to add?')                           # ask user for new fruit
